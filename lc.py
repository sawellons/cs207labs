...
class LightCurve:
    ...
    def timeseries(self):
        ...
        for i in range(0,len(self._time)):
            ts.append((self._time[i],self._amplitude[i]))
        return ts
    ...
    with open(filename) as fp:
        for line in fp:
            if line.find('#')!=0:
                ...
                elif len(facetvals)==0:
                    facetvals=line.strip().split()
    
    if len(facetnames) > 0:
        facetnames[0] = facetnames[0][1:]
        facetvals[0] = facetvals[0][1:]
    for i in range(0,len(facetnames)):
        facetdict[facetnames[i]] = facetvals[i]
